...
output <- "~/6. VR Study/VR_Method/VR_Method"
...
#myList <- list.dirs(path = "/Users/rasmus/Desktop/VR_Method/RAW VR")[-1]
myList <- list.dirs(path = "~/6. VR Study/VR_Method/VR_Method/RAW VR")[-1] #not quite good to name a list "list" because it might get confusing in R at some point, so I changed it to myList, [-1] because the first position listed is the main folder which is not needed here 
myFolders <- list.dirs(path ="~/6. VR Study/VR_Method/VR_Method/RAW VR", full.names = FALSE)[-1]
...
for (i in 1:length(myList))
{
  ...
  DF$id[i] <- abs(parse_number(myFolders[i])) # extract participant number from the file name
...
for (i in 1:length(myList))
{
  ...
  # Extract Participants number
  DF$id[i] <- parse_number(myList[i]) #extract participant number from the name of the file
...
#------------REACTIONAL TIME COEFFICIENT OF VARIANCE ---------------
...
#----NORMALITY FOR RT_md ------
#Info about skreness and kurtosis assessments: https://www.discoveringstatistics.com/repository/exploringdata.pdf 
...
p4 <- ggplot(summary_stats, aes(x = stimuli_type, y = mean, fill = cost_benefit)) +
  ...
  labs(x = "Stimuli Type", y = "Reaction Time Difffrence", fill = "Attention")
