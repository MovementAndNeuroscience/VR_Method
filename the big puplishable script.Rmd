...
ATTENTION <- data.frame(ATTENTION, cost_ben_val = rep(NA, nrow(ATTENTION)))
...
t_test(cost_ben_val ~ condition , p.adjust.method = "bonferroni", paired = T)
...
t_test(cost_ben_val ~ stimuli_type , p.adjust.method = "bonferroni", paired = T)
...
ATTENTION %>%
  group_by(stimuli_type,stimuli_compatibility) %>%
  t_test(corrects ~ condition , p.adjust.method = "bonferroni", paired = T)
...
# plotting all the permutations for each condition. 
  ggplot(ATTENTION, aes(x = stimuli_type, y = RT, fill = stimuli_compatibility)) +
  geom_violin(trim = FALSE, alpha = 0.5, position = "dodge") +
  geom_jitter(alpha = 0.9, size = 1, shape = 21, width = 1) +
  facet_wrap(~condition) +
  labs(x = "Stimuli Type", y = "Search Time", fill = "Stimuli Compatibility", color = "Stimuli Compatibility")+
  theme_bw()+ 
